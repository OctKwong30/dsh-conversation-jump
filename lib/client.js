...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const BUTTON_CSS = `
.dsh-jump-wrap { display: inline-flex; width: 0; height: 0; overflow: visible; }
/* 浮动触发按钮：会话列左缘（应用左侧栏右侧）垂直居中的圆形悬浮钮，
   位置由 JS 实测左缘后内联写入 left。 */
.dsh-jump-fab {
  ...
  box-shadow: 0 4px 14px rgba(0,0,0,.14); z-index: 9900;
}
.dsh-jump-fab:hover {
...
.dsh-jump-panel {
  /* 锚定会话区左侧：按钮右侧弹出、垂直居中；left 由 JS 实测左缘后内联写入。 */
  position: fixed; top: 50%; transform: translateY(-50%); width: 340px;
  max-width: calc(100vw - 32px); max-height: min(72vh, calc(100vh - 32px)); box-sizing: border-box;
  background: var(--dsw-alias-bg-overlay, #ffffff);
  border: 1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.1));
...
.dsh-jump-set { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; padding: 8px 0; }
...
/* 模式解释行：跟随当前选中模式的说明文案，置于控件行下方整行展示 */
.dsh-jump-set-note {
  flex-basis: 100%; margin: 2px 0 0; padding: 0 0 6px;
  font-size: 11px; line-height: 1.6; color: var(--dsw-alias-label-secondary, #6b7280);
}
...
`;
     ...
     *  每个选项带悬浮提示；控件行下方整行展示跟随当前模式的解释文案。 */
    const MODE_NOTE = {
      detailed: '详细模式：目录逐条列出会话中的每个节点 —— 用户消息、助手每步回复、每次工具调用、命令、压缩摘要、错误等，配类型徽标与文本摘要；适合精确定位到某一步。',
      concise: '简洁模式：目录只保留「对话骨架」—— 每个用户问题一条、每个回合的一次完整回复一条，中间的工具调用 / 上下文 / 压缩等节点全部隐藏；适合快速回看问答脉络。'
    };
    const MODE_TIP = {
      detailed: '逐节点列出：用户 / 助手 / 工具 / 命令 / 系统全部可见，支持按类型筛选',
      concise: '只列用户问题 + 回合回复，隐藏中间节点（工具调用 / 压缩等）'
    };
      ...
      return React.createElement('div', { className: 'dsh-jump-set' },
        ...
        React.createElement('div', { className: 'dsh-jump-set-control', role: 'radiogroup', 'aria-label': '对话节点显示模式' },
          ...opts.map((entry) => React.createElement('button', {
            ...
            title: MODE_TIP[entry[0]],
            className: 'dsh-jump-set-opt' + (mode === entry[0] ? ' active' : ''),
            onClick: () => writeMode(entry[0])
          }, entry[1]))
        ),
        React.createElement('div', { className: 'dsh-jump-set-note' }, MODE_NOTE[mode] || MODE_NOTE.detailed)
      );
    }
    ...
    // ---------- 会话列左缘定位 ----------
    // 悬浮钮与面板锚定在会话列左缘（应用左侧栏右侧）垂直居中。会话头部与
    // 聊天滚动区是兄弟分支（向上找滚动容器会命中其它元素），因此复用产品
    // 自带的 [data-chat-anchor-key] DOM 锚点契约：从当前可见的聊天行向上找
    // overflow-y 滚动容器 —— 即聊天滚动区，处于会话列内、应用左侧栏右侧，
    // 其左缘即目标锚线。左侧栏开合 / 宽度变化会改变该滚动区几何，通过对
    // 该滚动区自身的 ResizeObserver + window resize 重测跟随。
    ...
    function findVisibleAnchor(selector) {
      ...
      for (const row of rows) if (isVisibleAnchor(row)) return row;
      return rows.length ? rows[0] : null;
    }

    function measureChatLeft() {
      if (typeof document === 'undefined') return null;
      const rows = document.querySelectorAll('[data-chat-anchor-key]');
      for (const row of rows) {
        if (!isVisibleAnchor(row)) continue; // 隐藏视图（其它标签页）的行
        let el = row.parentElement;
        while (el != null && el !== document.body) {
          const cs = getComputedStyle(el);
          if (cs.overflowY === 'auto' || cs.overflowY === 'scroll') {
            const r = el.getBoundingClientRect();
            // left=0 合法（窄屏侧栏响应式收起、会话列贴视口左缘）；
            // 从锚点行向上找到的第一个滚动容器就是聊天滚动区，无误判来源。
            if (r.width > 0 && r.left >= 0) return Math.round(r.left);
          }
          el = el.parentElement;
        }
      }
    ...
    function jumpTo(key) {
      ...
      const row = findVisibleAnchor(selector);
    ...
    function NodeJumpButton(props) {
      ...
      // 会话列左缘测量：复用聊天行锚点定位聊天滚动区（与 jumpTo 同一契约），
      // ResizeObserver 观察滚动区自身（左侧栏开合/宽度变化 → 滚动区几何变化）
      // + window resize；空会话无锚点行时延迟重试。锚点行随消息进出出现 /
      // 消失（首条消息发送后才有行），故用 MutationObserver 盯 document.body
      // 子树变化兜底重测，并在节点数变化时重挂 ResizeObserver。
      const [chatLeft, setChatLeft] = React.useState(null);
      React.useEffect(() => {
        const remeasure = () => {
          const v = measureChatLeft();
          setChatLeft((prev) => (prev === v ? prev : v));
        };
        remeasure();
        let ro = null;
        let retry = null;
        const findScroller = () => {
          ...
          const rows = document.querySelectorAll('[data-chat-anchor-key]');
          for (const row of rows) {
            if (!isVisibleAnchor(row)) continue;
            let el = row.parentElement;
            while (el != null && el !== document.body) {
              const cs = getComputedStyle(el);
              if (cs.overflowY === 'auto' || cs.overflowY === 'scroll') {
                const r = el.getBoundingClientRect();
                if (r.width > 0 && r.left >= 0) return el;
              }
              el = el.parentElement;
            }
          }
        ...
        // 兜底：锚点行出现/消失（首条消息、标签页切换）时重测并重挂 observer。
        let anchorCount = document.querySelectorAll('[data-chat-anchor-key]').length;
        const mo = new MutationObserver(() => {
          const n = document.querySelectorAll('[data-chat-anchor-key]').length;
          if (n === anchorCount) return; // 与锚点行无关的 DOM 变更，忽略
          anchorCount = n;
          remeasure();
          attach(findScroller());
        });
        mo.observe(document.body, { childList: true, subtree: true });
        if (ro == null) retry = window.setTimeout(remeasure, 800);
        return () => {
      ...
      // 左缘未测得（极少数无滚动容器场景）时隐藏悬浮钮，避免错位。
      const fabLeft = chatLeft != null ? chatLeft + 40 : null;

      // Esc 关闭面板（焦点在面板或悬浮钮内时）；面板外不抢 Esc。
      React.useEffect(() => {
        if (!isOpen) return undefined;
        const onKey = (e) => {
          if (e.key !== 'Escape') return;
          const t = e.target;
          if (t instanceof Element && t.closest('.dsh-jump-panel, .dsh-jump-fab') == null) return;
          e.preventDefault();
          e.stopPropagation();
          setIsOpen(false);
        };
      ...
      const children = [React.createElement('button', {
        ...
        className: 'dsh-jump-fab' + (isOpen ? ' active' : ''),
        title: '对话节点跳转', 'aria-label': '对话节点跳转',
        style: fabLeft != null
          ? { left: fabLeft + 'px', visibility: 'visible' }
          : { visibility: 'hidden' },
      ...
      if (isOpen) {
        ...
        children.push(React.createElement('div', {
          key: 'panel', className: 'dsh-jump-panel',
          style: fabLeft != null
            ? { left: (fabLeft + 40) + 'px', visibility: 'visible' }
            : { visibility: 'hidden' }
        }, ...panelChildren));
      }
